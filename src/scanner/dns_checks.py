...
async def check_dns(url: str) -> list[dict]:
    ...
    # DNS resolution
    try:
        ips = await asyncio.get_event_loop().run_in_executor(None, lambda: socket.getaddrinfo(host, None))
    ...
    if discovered:
        risky = [
            ...
            for s in discovered
            if any(
                kw in s for kw in ["admin", "staging", "test", "dev", "internal", "jenkins", "git", "db", "database"]
            )
        ]
...
async def _check_dns_txt(domain: str, findings: list):
    ...
    try:
        # Check SPF
        await asyncio.get_event_loop().run_in_executor(None, lambda: socket.getaddrinfo(domain, None))
...
async def _enumerate_subdomains(domain: str) -> list[str]:
    ...
    async def _probe(sub: str):
        fqdn = f"{sub}.{domain}"
        try:
            await asyncio.get_event_loop().run_in_executor(None, lambda: socket.gethostbyname(fqdn))
            return fqdn
