...
DANGEROUS_HEADERS = {
    "Server": ("info", "Server header exposes software version", "Remove or obscure the Server header to reduce information leakage."),
...
async def check_ssl(url: str) -> list[dict]:
    ...
    if parsed.scheme != "https":
        findings.append({
            ...
            "category": "ssl",
            "title": "No HTTPS",
            "description": f"Target uses HTTP instead of HTTPS. All traffic is unencrypted.",
    ...
    for port in open_ports:
        ...
        findings.append({
            ...
            "description": f"Port {port} ({service}) is open and accepting connections."
                          + (f" This service should not be publicly exposed." if is_risky else ""),
            "evidence": f"TCP connect to {host}:{port} succeeded",
            "recommendation": f"Restrict access to port {port} via firewall rules." if is_risky
            ...
            for cookie_str in cookies:
                ...
                if "secure" not in lower:
                    findings.append({
                        ...
                        "description": f"Cookie may be sent over HTTP.",
...
SENSITIVE_PATHS = [
    ...
    "/admin/", "/admin/login", "/api/", "/api/docs", "/api/swagger",
    "/graphql", "/.well-known/security.txt", "/robots.txt", "/sitemap.xml",
    "/server-status", "/server-info", "/.htaccess", "/web.config",
    "/phpinfo.php", "/debug/", "/actuator/health", "/actuator/env",
    "/.DS_Store", "/backup/", "/dump.sql", "/database.sql",
...
async def check_paths(url: str) -> list[dict]:
    ...
    for path, status, size in results:
        ...
        if status == 200 and size > 0:
            ...
            is_critical = any(p in path for p in [".env", ".git", "dump.sql", "database.sql", "phpinfo", "actuator/env"])
